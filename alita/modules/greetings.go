...
func claimRecentJoinProcessing(chatID, userID int64) bool {
	...
	if cache.GetMarshal() != nil {
		if exists, _ := cache.GetMarshal().Get(cache.Context, key, new(bool)); exists != nil {
			return false
		}
		if err := cache.GetMarshal().Set(cache.Context, key, true, store.WithExpiration(recentJoinProcessTTL)); err == nil {
			return true
		}
...
func (moduleStruct) joinRequestHandler(b *gotgbot.Bot, ctx *ext.Context) error {
	...
	case "accept":
		...
		_ = cache.GetMarshal().Delete(cache.Context, fmt.Sprintf("alita:pendingJoins:%d:%d", chat.Id, joinUser.Id))
	case "decline":
...
func (moduleStruct) loadPendingJoins(chatId, userId int64) bool {
	...
	alreadyAsked, err := cache.GetMarshal().Get(cache.Context, fmt.Sprintf("alita:pendingJoins:%d:%d", chatId, userId), new(bool))
...
func (moduleStruct) setPendingJoins(chatId, userId int64) {
	...
	_ = cache.GetMarshal().Set(cache.Context, fmt.Sprintf("alita:pendingJoins:%d:%d", chatId, userId), true, store.WithExpiration(5*time.Minute))
}
