...
func (tp TracingProcessor) ProcessUpdate(d *ext.Dispatcher, b *gotgbot.Bot, ctx *ext.Context) (err error) {
	...
	// context to context-aware repositories, and docs/architecture/caching.md
	// documents that polling and webhook updates carry it. It is created
	// unconditionally, not only when tracing is on.
