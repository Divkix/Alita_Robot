...
import {
	pathnameToSlug,
	stripTrailingSlash,
} from "./node_modules/starlight-links-validator/libs/path.ts";
import {
...
import {
	logErrors,
	validateLinks,
} from "./node_modules/starlight-links-validator/libs/validation.ts";
...
const StarlightLinksValidatorOptionsSchema = z.object({
...
type StarlightLinksValidatorUserOptions = z.input<
	typeof StarlightLinksValidatorOptionsSchema
>;

export default function starlightLinksValidatorPlugin(
	userOptions?: StarlightLinksValidatorUserOptions,
): StarlightPlugin {
	const options = StarlightLinksValidatorOptionsSchema.safeParse(
		userOptions ?? {},
	);

	if (!options.success) {
		throwPluginError(
			"Invalid options passed to the starlight-links-validator plugin.",
		);
	}
						...
						"astro:build:done": ({ dir, pages, assets }) => {
							...
							for (const [pattern, urls] of assets) {
								...
								for (const url of urls) {
									customPages.add(
										pathnameToSlug(
											url.pathname.replace(astroConfig.outDir.pathname, ""),
										),
									);
								}
							}
							...
							const hasInvalidLinkToCustomPage = logErrors(
								logger,
								errors,
								site,
							);
