...
### `/alita/db/` - Database Layer <Badge text="Core" variant="note" />

The database layer implements the repository pattern:

:::note
Each `*_db.go` file is self-contained per feature domain. It includes GORM model definitions, query functions (Get, Set, Delete), and cache invalidation calls. Always invalidate cache after writes.
:::

<FileTree>
- db/
  - db.go Database connection, GORM setup, models
  - cache_helpers.go Cache key generators, TTL constants
  - migrations.go Migration runner
  - optimized_queries.go Optimized SELECT queries
  - monitoring.go Database monitoring
  - user_db.go User storage operations
  - chats_db.go Chat settings operations
  - bans_db.go Ban-related operations
  - filters_db.go Filter storage
  - notes_db.go Notes storage
  - notes_settings_db.go Notes configuration
  - warns_db.go Warning system storage
  - warns_settings_db.go Warning configuration
  - greetings_db.go Greeting settings
  - locks_db.go Lock settings
  - blacklists_db.go Blacklist storage
  - antiflood_db.go Antiflood settings
  - captcha_db.go CAPTCHA settings
  - admin_db.go Admin settings
  - connections_db.go Connection settings
  - disabling_db.go Disabled commands
  - channels_db.go Channel settings
  - backup_db.go Backup operations
  - backup_types.go Backup data types
  - devs_db.go Developer settings
  - report_settings_db.go Report configuration
  - ...
</FileTree>

### `/alita/modules/` - Command Handlers <Badge text="Core" variant="note" />

Feature modules contain command handlers and business logic:

:::note[Non-module files]
Not every file in `modules/` is a feature module. Files like `helpers.go`, `moderation_input.go`, `callback_codec.go`, `callback_parse_overwrite.go`, `chat_permissions.go`, `connections_auth.go`, and `rules_format.go` are shared utilities used across modules.
:::
...
- modules/
  ...
  - pins.go /pin, /unpin, /pinned
  - purges.go Message purging
  - misc.go Miscellaneous commands
  - bot_updates.go Bot status tracking
  - antispam.go Anti-spam measures
  - formatting.go Markdown commands
  - backup.go Backup/import commands
  - reactions.go Reaction commands
  - pin.go (legacy, now pins.go)
</FileTree>

### `/alita/utils/chat_status/` - Permission Checking <Badge text="Security" variant="caution" />
