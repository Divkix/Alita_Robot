...
Not every file in `modules/` is a feature module. Files like `helpers.go`, `moderation_input.go`, `callback_codec.go`, `chat_permissions.go`, `connections_auth.go`, and `rules_format.go` are shared utilities used across modules.
:::
