...
1. **Create database operations** in `alita/db/{module}/repository.go` (following the domain-package pattern)
...
5. **Self-register in `init()`** using `RegisterLegacyModule("MyModule", priority, LoadMyModule)` — modules auto-load via `LoadAllModules()` in `alita/main.go`
