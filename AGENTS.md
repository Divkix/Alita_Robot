...
Never commit real tokens; base your `.env` on `sample.env`. When touching database logic, run `make psql-prepare` to sync cleaned migrations. Review `docker/` templates before deploying and ensure webhook URLs or API keys are provided through environment variables.
